...
class RobotDataset(Dataset):
    def __init__(self, csv_file=None, data_dict=None, sequence_length=50):
    ...
    def __len__(self):
        return len(self.angles) - self.sequence_length + 1
    
    def __getitem__(self, idx):
        end_idx = idx + self.sequence_length
        
        return {
            'angles': self.angles[idx:end_idx],
            'velocities': self.velocities[idx:end_idx],
            'accelerations': self.accelerations[idx:end_idx],
            'torques': self.torques[idx:end_idx]
        }
...
def main():
    ...
    save_dir = os.path.join(MODELS_DIR, f'pinn_rnn_model_{timestamp}')
    os.makedirs(save_dir, exist_ok=True)
    print(f"Saving results to: {save_dir}")
    
    # Load data
    sequence_length = 50  # Length of sequences for RNN
    full_dataset = RobotDataset(DATA_FILE, sequence_length=sequence_length)
    ...
    # Create data loaders
    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True)  # Smaller batch size for RNN
