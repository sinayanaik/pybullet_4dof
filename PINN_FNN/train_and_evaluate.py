...
class RobotDataset(Dataset):
    def __init__(self, csv_file, forward_only=True):
    ...
    def __getitem__(self, idx):
        X = torch.cat([self.q[idx], self.qdot[idx], self.qddot[idx]])
        y = self.tau[idx]
        return X, y
...
def compute_physics_loss(q, qdot, qddot, pred_tau):
    ...
    # Gravitational terms
    g_term1 = m1 * g * L1/2 * torch.cos(q[:, 0]) + \
              m2 * g * L1 * torch.cos(q[:, 0]) + \
              m2 * g * L2/2 * torch.cos(q[:, 0] + q[:, 1]) + \
              m3 * g * (L1 * torch.cos(q[:, 0]) + L2 * torch.cos(q[:, 0] + q[:, 1]) + \
                       L3/2 * torch.cos(q[:, 0] + q[:, 1] + q[:, 2]))
    
    g_term2 = m2 * g * L2/2 * torch.cos(q[:, 0] + q[:, 1]) + \
              m3 * g * (L2 * torch.cos(q[:, 0] + q[:, 1]) + \
                       L3/2 * torch.cos(q[:, 0] + q[:, 1] + q[:, 2]))
    
    g_term3 = m3 * g * L3/2 * torch.cos(q[:, 0] + q[:, 1] + q[:, 2])
    ...
    # Expected torques based on physics
    physics_tau1 = M11 * qddot[:, 0] + g_term1
    physics_tau2 = M22 * qddot[:, 1] + g_term2
    physics_tau3 = M33 * qddot[:, 2] + g_term3
...
class HyperparameterGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("PINN FNN Hyperparameters")
    ...
    def validate_params(self):
        try:
            ...
            for key, entry in self.entries.items():
                ...
                if key in ['hidden_size', 'num_layers', 'batch_size', 'epochs']:
                    value = int(value)
                params[key] = value
            ...
            if not (2 <= params['num_layers'] <= 5):
                raise ValueError("Number of layers should be between 2 and 5")
...
def train_and_evaluate(params):
    ...
    base_dir = os.path.join(MODELS_DIR, f"pinn_fnn_h{params['hidden_size']}_l{params['num_layers']}_lr{params['learning_rate']:.6f}_{timestamp}")
    os.makedirs(base_dir, exist_ok=True)
    
    # Load and split data
    dataset = RobotDataset(params['data_file'], forward_only=True)
    ...
    data_model = PINN_FNN(hidden_size=params['hidden_size'], num_layers=params['num_layers'])
    ...
    if params['physics_weight'] > 0:
        ...
        pinn_model = PINN_FNN(hidden_size=params['hidden_size'], num_layers=params['num_layers'])
